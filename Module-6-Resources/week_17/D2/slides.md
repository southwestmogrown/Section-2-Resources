...
The concept of "truthiness" refers to values that _evaluate_ to `True` when typecast as a Boolean.

"Truthy" values however are not necessarily _equal_ to `True`.
...
While these lists have the same _value_, they are two distinct objects in memory with different identities.
...
This is a space optimization mechanism called _interning_.
